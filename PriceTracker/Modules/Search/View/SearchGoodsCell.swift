...
final class SearchGoodsCell: UITableViewCell {
    static let reuseId = "SearchGoodsCell"
    
    var delegate: SearchGoodsCellDelegate?
    
    private let wrap = UIView()
    let image = UIImageView()
    let shopView = UIImageView()
    ...
    private func configureShopView() {
        image.addSubview(shopView)
        shopView.pinTop(to: image.topAnchor, 4)
        shopView.pinLeft(to: image.leadingAnchor, 4)
        shopView.setHeight(20)
        shopView.setWidth(20)
